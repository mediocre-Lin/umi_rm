...
import time
from modules.rm_zmq_client import RMClient
...
class RMController(mp.Process):
    """
    RM机械臂控制器
    基于Arx5Controller接口，使用RMClient与RM机械臂通信
    保持与Arx5Controller完全相同的接口和数据结构
    """
    ...
    def __init__(
        ...
        super().__init__(name="RMController")
        self.robot_ip = robot_ip
        self.robot_port = robot_port

        # 与Arx5Controller相同的输入队列结构
        example = {
        ...
        # 与Arx5Controller相同的接收键名映射
        # 注意：这里使用RMClient的属性名，但保持Arx5Controller的键名
        receive_keys = [
            ("ActualTCPPose", "tcp_pose"),  # RMClient的tcp_pose属性
            ("ActualQ", "joint_pos"),        # RMClient的joint_pos属性
            ("ActualQd", "joint_vel"),       # RMClient的joint_vel属性
            ("gripper_position", "gripper_pos"),  # RMClient的gripper_pos属性
        ]
        example = dict()
    ...
    def start(self, wait=True):
        ...
        if wait:
            self.start_wait()
        if self.verbose:
            print(f"[RMController] Controller process spawned at {self.pid}")
    ...
    def stop(self, wait=True):
        ...
        if wait:
            self.stop_wait()
        if self.verbose:
            print(f"[RMController] Controller process terminated at {self.pid}")

    def start_wait(self):
        print(f"[RMController] Waiting for controller process to be ready")
    ...
    def run(self):
        # 使用RMClient替代Arx5Client
        self.robot_client = RMClient(self.robot_ip, self.robot_port)
        self.robot_client.reset_to_home()
        time.sleep(1)
        
        # RM机械臂的增益设置（与Arx5Controller保持相同的接口）
        ...
        try:
            ...
            while keep_running:
                ...
                # 使用RMClient的set_tcp_pose方法
                self.robot_client.set_tcp_pose(pose_cmd, gripper_cmd)
                
                # 构建状态字典，保持与Arx5Controller相同的键名
                state = dict()
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.STOP.value:
                        ...
                        if self.verbose:
                            print(
                                f"[RMController] New pose target: {target_pose} duration {duration:.3f}s"
                            )
                        ...
                        self.ring_buffer.put(state)
                        if self.verbose:
                            print(f"[RMController] Reset to home")
                        ...
                        if len(self.waypoint_buffer) > 0:
                            ...
                            if command["target_time"] <= last_waypoint_time:
                                print(
                                    f"[RMController] Waypoint time {command['target_time']:.3f} is not in the future, skipping"
                                )
                                continue
                        ...
                        if len(self.waypoint_buffer) == 0:
                            print(
                                f"[RMController] No new waypoints to update trajectory"
                            )
                            continue
                        ...
                        if self.verbose:
                            print(
                                f"[RMController] latency: {best_latency:.3f}s, error: {errors.min():.3f}, time: {time.monotonic() - start_time:.3f}s"
                            )
                        # clear buffer
                        self.waypoint_buffer = []
                    else:
                        keep_running = False
                        print(f"[RMController] Unknown command {cmd}")
                        break
                # regulate frequency
                ...
                if iter_idx == 0:
                    ...
                    if self.verbose:
                        print("[RMController] Controller process is ready")
                iter_idx += 1

        finally:
            print("[RMController] Setting robot to damping")
            self.robot_client.set_to_damping()
            del self.robot_client
            self.ready_event.set()
            if self.verbose:
                print("[RMController] Controller process terminated")


# 为了兼容性，创建别名
Arx5Controller = RMController
