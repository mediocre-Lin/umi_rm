...
from modules.rm_controller import RMController
...
class RMEnv:
    """
    RM机械臂环境
    基于Arx5Env接口，使用RMController与RM机械臂通信
    保持与Arx5Env完全相同的接口和数据结构
    """
    ...
    def __init__(
        ...
        # required params
        output_dir,
        robots_config,  # list of dict[{robot_type: 'rm', robot_ip: XXX, obs_latency: 0.0001, action_latency: 0.1, tcp_offset: 0.21}]
        # env params
        frequency=20,
        # obs
        ...
        # 使用RMController替代Arx5Controller
        robots: List[RMController] = list()
        for rc in robots_config:
            this_robot = RMController(  # 使用RMController
                shm_manager=shm_manager,
